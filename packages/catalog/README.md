...
The root entrypoint contains portable YAML/CSV parsing, schemas, and entrypoint
generation. File-backed loading uses Node's filesystem APIs and is therefore
explicitly exported from `@lucid-agents/catalog/node`.
...
| `paymentProtocol` | `"x402"` or `"mpp"`  | No       | Payment rail for a priced item. Required when both rails are installed unless a default is supplied. |
...
```csv
key,name,description,price,network,paymentProtocol,meta_category,meta_model
weather,Weather Lookup,Get current weather,0.001,eip155:8453,x402,utility,gpt-4o-mini
translate,Translation,Translate text,0.002,,mpp,nlp,gpt-4o
sentiment,Sentiment Analysis,Analyze sentiment,0.001,,x402,nlp,gpt-4o-mini
```
...
| `paymentProtocol` | No       | `x402` or `mpp`.                                                                                   |
...
| `paymentProtocol` | `"x402" \| "mpp"` | No       | Default payment rail for items that do not specify their own.                                                                      |
...
| `paymentProtocol` | `"x402" \| "mpp"` | Default payment rail for items without one. |
...
When both x402 and MPP are installed, every priced generated entrypoint needs an explicit rail. Set `paymentProtocol` on each YAML/CSV item or provide a catalog-wide default; an item-level value wins.
