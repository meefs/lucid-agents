...
Hono and Express render a minimal endpoint directory as static, read-only HTML.
Each invoke and stream operation appears with its HTTP path, payment method,
network, and price. The page contains no schemas, raw Agent Card JSON, client
JavaScript, or browser invocation controls.
...
public model and render the same read-only table.
