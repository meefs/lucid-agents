...
<Callout type="warn">
  This command applies after `@lucid-agents/catalog` is published. As of the
  verification date, catalog is a Next workspace package and is not part of the
  clean-room Stable npm set.
</Callout>

The root export contains portable parsers, schemas, types, and entrypoint generation. File-backed loading uses Node filesystem APIs and is available explicitly from `@lucid-agents/catalog/node`.
...
| `paymentProtocol` | `"x402"` or `"mpp"`  | No       | Payment rail for a priced item. Required when both rails are installed unless the extension supplies a default. |
...
```csv
key,name,description,price,network,paymentProtocol,meta_category,meta_tier
ocr,OCR Scan,Extract text from images,0.50,,x402,vision,standard
embed,Text Embedding,Generate vector embeddings,0.02,,mpp,ml,basic
classify,Text Classifier,Classify text,0.10,,x402,nlp,basic
```
...
type CatalogExtensionOptions = {
  ...
  paymentProtocol?: 'x402' | 'mpp'; // Default payment rail
...
| `paymentProtocol` | `undefined`                                      | Default payment rail for items that do not specify their own.                   |
...
Combine with the MPP extension for built-in Tempo/Stripe or application-verified custom payments:
...
x402 and MPP can coexist on the same agent. When both are installed, set `paymentProtocol` on every priced catalog item or provide a catalog-wide default. An item-level value takes precedence over the extension default.
...
const items = agent.catalog?.items ?? [];
...
`solana:mainnet` and `solana:devnet` are legacy aliases accepted by the
payments package, not canonical CAIP-2 identifiers. Catalog parsing preserves
the configured string; use canonical values in new catalog data.
...
```typescript
// Node-only file-backed extension
export { catalog } from '@lucid-agents/catalog/node';
export type { CatalogRuntime } from '@lucid-agents/catalog/node';
