...
When using `wrapFetchWithPayment` directly, you control this limit:

```typescript
// Set max payment to $1 USDC
const x402Fetch = wrapFetchWithPayment(fetch, wallet, BigInt(1_000_000));
```
