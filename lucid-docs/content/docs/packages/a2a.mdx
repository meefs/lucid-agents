...
The A2A extension adds Agent Card discovery, direct invocation, streaming clients, and an owned asynchronous task state machine. HTTP task routes appear only when `a2a()` is installed.
...
`buildCard()` builds the A2A-owned portion directly. The HTTP manifest handler runs the complete manifest contribution pipeline, adding payments, identity, AP2, and other installed capabilities.
...
const owned = await agent.a2a.client.listTasks(
  card,
  access.accessToken,
  {
...
const conversation = await agent.a2a.client.listTasks(
  card,
  first.accessToken,
  { contextId: 'conversation-123' }
);
