...
# Install only the low-level packages your direct integration uses
bun add @x402/core @x402/fetch @x402/evm
```

Lucid Agents applications normally install `@lucid-agents/payments` and let the runtime own server-side verification. Add `@x402/hono`, `@x402/express`, or `@x402/next` only for a separate raw x402 integration; Lucid's adapters do not require them.
...
const config: PaymentsConfig = {
  ...
  facilitatorUrl: 'https://YOUR_FACILITATOR_URL',
};
...
// After: import each type from the scoped package that owns it.
import type { Network } from '@x402/core/types';
import type { RouteConfig } from '@x402/core/server';
```
...
These import changes apply only when your application uses the raw framework middleware directly. Lucid Agents runtimes should migrate to the unified extension shown below.
...
if (!paymentContext.fetchWithPayment) {
  throw new Error('No compatible buyer wallet is configured');
}
 ...
 *   - PAYMENTS_NETWORK - CAIP-2 identifier (for example, eip155:84532)
 */
```
...
2. **Update networks**: Convert to CAIP-2 format (for example,
   `"base"` → `"eip155:8453"`).
3. **Update imports**: Change import paths from `x402/types` to the owning
   scoped package.
4. **Update tests**: Mock v2 facilitator responses and use CAIP-2 networks.

### Optional (If Using Raw x402 Client)

5. **Update client creation**: Create `x402Client`, then register the selected
   exact scheme helper.
6. **Update signer**: Supply the chain-compatible signer required by that
   scheme helper.
...
- [Call a Paid Service](/docs/buy/call-paid-service)
- [Payment Policies and Budgets](/docs/buy/policies-budgets)
...
- Review the [payments changelog](https://github.com/daydreamsai/lucid-agents/blob/master/packages/payments/CHANGELOG.md)
