...
- 4528ad8: Replace generated service dashboards with a minimal read-only endpoint table showing each invoke and stream path, payment method, and price. Remove raw JSON, schema editors, browser invocation controls, wallet UI scaffolding, and their client dependencies from generated Next.js and TanStack UI projects.

## 4.1.0

### Minor Changes

- 8066f57: Add secure, preview-first Cloudflare deployment for newly generated blank Hono
  agents, including a versioned allowlist manifest, a separate Worker entry,
  redacted environment upload, CI safeguards, and deployed-origin verification.
- 5f35b68: Add typed Dossier, Folio, and Console service UI presets with bounded color and
...
- c21990b: Replace generated dashboards and the portable landing page with a shared agent
  service storefront. Agent Cards now publish per-entrypoint payment protocol,
  network, and SIWX requirements; HTTP exposes a canonical public service-page
  model; and generated Next and TanStack applications share accessible invoke,
  stream, payment, and A2A task interactions.

### Patch Changes

- 17fa5eb: Use USD decimal prices and canonical `PAYMENTS_*` configuration names in generated and repository examples.
...
- 583dc87: Add packed-project CI coverage for every generated adapter, make the landing page
  hydrate safely, and keep the optional Stripe dependency compatible with framework
  bundlers.
...
- Refresh the generated Next.js and TanStack landing pages with a cleaner terminal-style layout and align CLI documentation with the current package names and release workflow guidance.
