...
<Callout type="warn">
  ...
  `mppx` 0.4.11 and implements the documented Tempo, Stripe, and custom-verifier
...
The package includes mppx. Tempo requires Viem `>=2.47.5`, declared as a peer dependency.
...
`tempo.server()` is materialized as a native mppx charge method. mppx verifies the echoed HMAC challenge, credential schema, transaction, amount, recipient, and settlement before Lucid admits the request. Native mppx sessions require a server signing account that Lucid's current `TempoServerConfig` does not expose; use a custom method and verifier for session intents.
...
an application verifier. Lucid does not materialize a native Tempo session
without the server signing account required by mppx 0.4.
