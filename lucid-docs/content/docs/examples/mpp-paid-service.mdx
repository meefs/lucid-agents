...
MPP_TEMPO_CURRENCY=0x20c0000000000000000000000000000000000000  # pathUSD on Tempo
      ...
      config: {
        methods: [
          tempo.server({
            ...
            recipient: process.env.MPP_TEMPO_RECIPIENT ?? '0xYourAddress...',
          }),
        ],
        currency: 'usd',
...
addEntrypoint({
  ...
  description: 'Premium generation — $1.00 via Tempo',
  price: '1.00',
  paymentProtocol: 'mpp',
  metadata: {
    mpp: {
...
4. **Metadata overrides** (`premium-generate`) — restricts the accepted method to `tempo` and adds a human-readable description to the challenge.
...
- [Call a Paid Service](/docs/buy/call-paid-service) - x402 client journey
